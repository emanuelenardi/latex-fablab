...
::: {layout-ncol=3}
![Julia Fox](assets/images/pronunciation/Julia-Fox-latex.webp){fig-align="center" height="450px"}

![Maya Jama](assets/images/pronunciation/Maya-Jama-latex.webp){fig-align="center" height="450px"}

![Timothée Chalamet](assets/images/pronunciation/Timothee-Chalamet-latex.webp){fig-align="center" height="450px"}
:::
...
## Distribuzione di TeX {.smaller #tex-distro}
...
::: {layout-ncol=2}
![3a. Solo `pip` (*prima* della deselezione)](assets/images/install-python/03a-optional-justpip.png)

![3b. Solo `pip` (*dopo* la deselezione)](assets/images/install-python/03b-optional-justpip.png)
:::
...
::: {layout-ncol=3}
![1. Proprietà di sistema](assets/images/install-python/06-system-properties.png)

![2. Variabili d'ambiente](assets/images/install-python/07-environment-variables.png)

![3. Modifica le variabili](assets/images/install-python/08-edit.png)
:::
...
::: {layout-ncol=2}
![1. "Esegui comunque" nascosto](assets/images/install-texlive/windows/00-smartscreen-more-info.png){width=400px}

![2. "Esegui comunque" visibile](assets/images/install-texlive/windows/00-smartscreen.png){width=400px}
:::
::: {.nonincremental}
...
::: {layout-ncol=2}
![1. Non hai i permessi di scrittura sulla cartella](assets/images/install-texlive/linux/00-texlive-user.png)

![2. Hai i permessi di scrittura sulla cartella](assets/images/install-texlive/linux/01-texlive-sudo.png)
:::
...
## Controllo dell'installazione {{< fa brands windows >}} {{< fa brands linux >}} {{< fa brands apple >}} {#controllo-installazione}
...
## Controllo dell'installazione {{< fa brands windows >}} {{< fa brands linux >}} {{< fa brands apple >}} {#hello-world}

1. crea un semplice documento:
```{.latex filename="main.tex"}
% arara: pdflatex
...
\end{document}
```
2. apri un terminale e digita `arara main`
$$
\rm Hello\ \LaTeX!
$$
...
<!-- # Documentazione e utilities {.smaller}
...
## Shortcut search engine su Chrome

- [How to add a custom search engine to Chrome](https://zapier.com/blog/add-search-engine-to-chrome/)

![](assets\images\chrome\ctan.png)

```txt
https://ctan.org/search/?phrase=%s
```

## Powertoys {{< fa brands windows >}} {#powertoys .smallest}

- [Microsoft learn](https://learn.microsoft.com/it-it/windows/powertoys/)

Alcune utilità che ci possono essere utili:

| Strumento | Shortcut |
|---------|:-----|
| Advanced Paste | {{< fa brands windows >}} `+ shift + V` |
| Color Picker | {{< fa brands windows >}} `+ shift + C` |
| FancyZones | {{< fa brands windows >}} `+ shift + ò` |
| File Explorer add-ons utility | 
| Quick Accent |  |
| Screen Ruler | {{< fa brands windows >}} `+ shift + M` |
| Text Extractor | `ctrl + shift + T` |

## Mathpix

- [Image to LaTeX](https://mathpix.com/image-to-latex)
- [Mathpix Extension for Chrome](https://mathpix.com/extension-downloads) -->

<!-- # Visual Studio Code {#vscode .white .smaller data-background-image="assets/images/hero/vscode.jpg" data-background-size="cover"}

Impostazione di un editor di testo.

## Ottimizzare il flusso di lavoro su Visual Studio Code {.smallest #ottimizzazione-flusso}
...
- `Preferences: Open Keyboard Shortcuts` ([see my shortcuts](#my-keyboard-shortcuts))
...
:::{.larger}
```{.json filename="keybindings.json" code-line-numbers="|4-9|12-17|20-25|28-33"}
...
<!-- #### Esempio di routine {#arara-routine}
...
<!-- # Perché la tipografia è _importante_? {.smaller #tipografia data-background-image="assets/images/hero/typography.jpg" data-background-size="cover"}
...
- `tabu`, `tabularx`, `booktabs`, `array`, `multirow`, `longtable` → [...`tabularray`, `tblr-extras`]
...
# Struttura del progetto {.smaller #project-structure}

Da quali parti è composta una tesi?

## Struttura generale di una tesi {.smallest #struttura-tesi}
...
<!-- ## Struttura del nostro progetto {#struttura-progetto}

:::{.code .larger}
```{.txt filename="Albero del progetto" code-line-numbers="|52-53|50-51|47-49|53|52|24|23|22|2|3-4|5-13|6-9|10-12|13|14|15|16-18|19-20|21|54|"}
.
├── assets/
│   ├── appendixes/
│   │   └── A*-appendix-name.tex
...
Codifica dei caratteri, impostazione della lingua, margini del documento, migliorare la scrittura, come fare i commenti. -->

<!-- ## Codifica dei caratteri {.smaller #package-fontenc}

```{.latex filename="preamble.main.sty" code-line-numbers="|1|2|"}
\usepackage[T1]{fontenc}% font encoding
%\usepackage[utf8]{inputenc}% default from 2018
```

- [fontenc vs inputenc](https://tex.stackexchange.com/questions/44694/fontenc-vs-inputenc)
- [Why should I use `fontenc`?](https://tex.stackexchange.com/questions/664/why-should-i-use-usepackaget1fontenc)
- [Is there any reason to use `inputenc`?](https://tex.stackexchange.com/questions/370278/is-there-any-reason-to-use-inputenc) -->
...
<!-- ## Margini del documento {.smaller #package-geometry}

::: {layout-ncol=2}
```{.latex filename="settings/set-geometry.sty" code-line-numbers="|1,8|2|3|4|5|6|7|"}
\usepackage[
    a4paper,
    % showframe,
    inner = 1.5cm,
...
![Layout package](assets/images/geometry/layout-dimensions.png){height="600"}
:::

- [Page size and margins](https://www.overleaf.com/learn/latex/Page_size_and_margins) -->

<!-- ## Gestire la spaziatura {#package-parskip}
```{.latex filename="settings/set-spacing.sty" code-line-numbers="|1-6|8-10|"}
\usepackage[
    skip=0.2cm,   % default: .5\baselineskip plus 2pt explicitly specify the vertical space between paragraphs
    tocskip=0pt,  % default: \parskip
    indent=0pt,
...
## Migliorare la scrittura {#package-microtype}

```{.latex filename="settings/set-typography.sty"}
\usepackage[
...
<!-- ## Come fare i commenti {#package-comment .smaller}

- [Commenting out large sections](https://tex.stackexchange.com/questions/17816/commenting-out-large-sections)
- Commanta con `Ctrl + ù`/`Ctrl + Shift + ù` su Visual Studio Code
...
<!-- # Impaginare le figure

Posizionamento delle figure, figure a metà pagina, figure nel testo.

## Il pacchetto `graphicx` {.smaller #package-graphicx}

```{.latex filename="settings/set-graphics.sty" code-line-numbers="||"}
\usepackage[draft]{graphicx}
```

### Comandi utili {.smaller #package-graphicx-commands}
...
<!-- ### Impostazioni del pacchetto {.smaller #package-graphicx-options}

```latex
\graphicspath{⟨dir-list⟩}
```
This optional declaration may be used to specify a list of directories in which to
search for graphics files. The format is as for the LATEX2ε primitive `\input@path`, a list of directories, each in a `{}` group (even if there is only one in the list). For example: `\graphicspath{{eps/}{tiff/}}` would cause the system to look in the subdirectories `eps` and `tiff` of the current directory.
```{.latex filename="settings/set-graphics.sty"}
% NOTE define absolute path to images
\graphicspath{
    % NOTE percorsi dal main
    {assets/images/}% generic
    {assets/images/logo/}% university logo
    {assets/images/01/}% 01-introduction
    {assets/images/02/}% 02-state-of-the-art
    {assets/images/03/}% 03-exploration
    {assets/images/04/}% 04-architecture
    {assets/images/05/}% 05-implementation
    {assets/images/06/}% 06-schools
    {assets/images/07/}% 07-local-community
    {assets/images/08/}% 08-implementation-online
    {assets/images/papers/articoloFIE/}% articolo
    {assets/images/papers/PRIMM}% articolo
}
``` -->

<!-- ## Il pacchetto [`caption`](https://ctan.org/pkg/caption) {.smaller #package-caption}

Personalizzazione delle didascalie degli ambienti "mobili"

```{.latex filename="settings/set-graphics.sty" code-line-numbers="|1|2-8|10-14|16-20|"}
...
## Il pacchetto `tabularray` {#package-tabularray .smaller}

The `LaTeX3` package tabularray provides a new way to typeset tables in a keyval way. This allows the separation of content and style

- [Guides for typesetting tables with the tabularray package](https://tex.stackexchange.com/questions/629480/)

```latex
\usepackage{tabularray}
\UseTblrLibrary{amsmath, booktabs, counter, diagbox, functional, siunitx, varwidth}
\UseTblrLibrary{nameref, zref}% compatibily with longtblr environment
\UseTblrLibrary{hook, html}% experimental
``` -->

<!-- ### Esempio {#package-tabularray-example .smaller}

```{.latex filename=tabularray-example.tex code-line-numbers="|2-4|6-12|7|8|9|10|12|13-19|"}
\begin{table}
    \centering
    \caption{Esempio di didascalia.}
    \label{tab:tabularray-example}
    \begin{tblr}{
        colspec={@{} *{8}{c} @{}},
...
```{.latex filename=settings/set-tables.sty}
% NOTE aligning decimal points in table
% NOTE numbers in text mode
\usepackage{siunitx}
``` -->

<!-- # Scrivere la matematica {#math}

## Materiale rilevante

- [Why is  `\[...\]` preferable to `$$...$$`?](https://tex.stackexchange.com/questions/503/why-is-preferable-to)
- [witharrows](https://ctan.org/pkg/witharrows)
- [siunitx](https://ctan.org/pkg/siunitx) -->

<!-- ## Ambienti per la matematica {#math-envs}

### witharrows {#package-witharrows .smaller}

- [witharrows](https://ctan.mirror.garr.it/mirrors/ctan/macros/generic/witharrows/witharrows.pdf)

```latex
\[\begin{WithArrows}
    f(n) &= 3 n^2 + 7 n^2 \Arrow{\(\forall n \geqslant 1\), effettuo una maggiorazione} \\
...
<!-- # Scrivere codice {#code .smaller}

Scrivere codice nel testo, importare file esterni e personalizzarne l'apparenza.

## Importare il pacchetto `minted` {#package-minted .smaller}
...
Importiamo il pacchetto in `settings/set-code.sty`:
```latex
...
<!-- ### Opzioni utili per il pacchetto `minted` {#package-minted-options .smaller}

:::{.code .larger filename="settings/set-code.sty"}
```latex
...
<!-- ### Come aggiungere una didascalia {#package-minted-caption .smaller}
...
- Basandoci su [questa domanda](https://tex.stackexchange.com/questions/452150/) utilizziamo il pacchetto [`tcolorbox`](https://ctan.org/pkg/tcolorbox).
...
<!-- # Formattare gli algorirmi {#algorithms}

[be confused with me](https://tex.stackexchange.com/questions/229355/)

## Pacchetti ed ambienti per gli algoritmi {#choosing-package-algorithms .smallest}

| pacchetto | ultimo aggiornamento |
|---------|:-----|
| algorithms | August 24, 2009 |
| algorithmicx | April 27, 2005 |
| algpseudocodex | April 17, 2023 |
| algorithm2e | July 18, 2017 |
algcompatible

| pacchetto | ambiente che definisce |
|---------|:-----|
| algorithms | algorithmic, algorithm |
| algorithmicx | algpseudocode, algpascal, algc | -->

<!-- ## {#package-algorithm2e .smaller}

Il pacchetto [algorithm2e](https://ctan.org/pkg/algorithm2e) è quello più versatile. -->

<!-- ## Bonus tip: Il pacchetto `forest`

[forest](https://ctan.org/pkg/forest) -->

<!-- # La bibliografia {#bibliography .smaller}

Creazione, gestione e personalizzazione della bibliografia.

## Come creare un bibliografia {#bibliography-creation .smaller}

::::{.columns}
:::{.column width="65%"}
1. crea un file `bibliography.bib`:
   a. aggiungi le voci bibliografiche nel formato [formato richiesto da BibTeX](#bibtex-entries);
2. all'interno di: `set-bibliography.sty`:
   a. importa il package `biblatex`;
   b. collega il database `.bib` da utilizzare: `\addbibresource{<nome>.<ext>}`.
3. includi `set-bibliography.sty` in `thesis.preamble.sty`;
4. stampa la bibliografia con `\printbibliography`;
5. aggiungi il passaggio di compilazione ad `arara`:
   a. `% arara: biber`
6. personalizza lo stile della bibliografia con `\bibliographystyle{<style>}`; -->
<!-- 7. personalizza la bibliografia con `\DeclareFieldFormat{<field>}{<format>}`. -->
<!-- :::
:::{.column width="35%"}
```{.txt code-line-numbers="false"}
.
...
├── settings/
│   ├── ...
│   ├── ✏️set-bibliography.sty
│   └── ...
├── ✏️thesis.preamble.sty
└── ✏️thesis.tex
```
:::
:::: -->

<!-- ## BibTEX bibliography entries {#bibtex-entries-1 .smallest}

Esempi di tipo di documenti:
...
- `@conference` an article that has been included in conference proceedings;
- `@inbook` a section, such as a chapter, or a page range within a book.: the chapter of interest is identified by a chapter number;
- `@incollection` is for a contributed section in a book, so would have its own `author` and `title`. The actual title of the entire book is given in the `booktitle` field;
...
<!-- ## Come fare una citazione {#package-biblatex-citation .smaller}
...
- `\supercite(s)`
- `\autocite`
- `\citeauthor/title/year/date/url`

- [Documentazione di biblatex](https://ctan.mirror.garr.it/mirrors/ctan/macros/latex/contrib/biblatex/doc/biblatex.pdf) 3.9 Citation Commands
- [Biblatex Cheat Sheet](https://tug.ctan.org/info/biblatex-cheatsheet/biblatex-cheatsheet.pdf)
- [Quale comando usare?](https://tex.stackexchange.com/a/162370) -->

<!-- ## Opzioni di stile della bibliografia {#package-biblatex-options .smaller}

`\usepackage[<options>]{biblatex}`, dove `options` posso essere:

- `backend`: `biber`
- `style`: `numeric`
- `sorting`: ordinati per `name`, `year`, `e` e/o `title`
- `citestyle`: [Documentazione di biblatex](https://ctan.mirror.garr.it/mirrors/ctan/macros/latex/contrib/biblatex/doc/biblatex.pdf) 3.3.1 Citation Styles
- `maxbibnames` / `minbibnames`
- `sorting`:
- `uniquename`: disambiguate using initials or full names, as required;
- `uniquelist`: disambiguate the labelname list. -->

<!-- ## Differenze fra Overleaf e VSCode {#bibliography-overleaf-vs-vscode .smaller}

Processing LATEX reference lists or other forms of cross-referencing, such as indexes, requires multiple runs of software—including the TEX engine (e.g., pdflatex) and associated programs such as BibTEX, makeindex, etc. As mentioned above, Overleaf handles all of these mulitple runs automatically, so you don't have to worry about them. -->

<!-- ## Tipi di file {#package-biblatex-exts .smaller}

- `bib`: used to describe bibliographical references. When used together with LaTeX and BibTeX, it generates a file with the `.bbl` extension;
- `bbl`: a `tex` file with a different extension containing your references **formatted according to the bibliography style chosen in your `.tex` file** through the `\bibligraphystyle{}` command. LaTeX then automatically inserts the contents of the `bbl` file in the exact place you call the `\bibliography{}` command in your document. -->
<!-- - `bcf`: **biber control file**, contains the information biber needs to process your bibliography -->
<!-- - `blg`: **BibTeX log file**, contains information about any errors, warnings, or other messages generated during the processing of the bibliography
- `bst`: **BibTeX style file**, specify the formatting and style of the bibliography
- `cfg`: **configuration file**, contains information about the formatting of the bibliography -->
<!-- - `cbx`: **biblatex citation style file**, specify the formatting and style of the citations; -->
<!-- - `bbx`: **biblatex bibliography style file**, specify the formatting and style of the bibliography -->

<!-- ### Esempi di modifiche alla bibliografia {#bibliography-examples .smaller}

```latex
% https://tex.stackexchange.com/questions/10682/suppress-in-biblatex
\renewbibmacro{in:}{}

% https://tex.stackexchange.com/questions/620082/in-biblatex-how-to-change-pp-to-p-when-referring-to-pages-of-references
...
Creare glossari ed elenchi di acronimi.

## Il package glossaries {#package-glossaries .smaller}

Il pacchetto [`glossaries`](https://ctan.org/pkg/glossaries) supporta acronimi e glossari multipli e prevede il funzionamento in più lingue (tramite [`babel`](https://ctan.org/pkg/babel)).

# Definire comandi {.smaller}

Come creare ed utilizzare comandi personalizzati per le proprie esigenze.
